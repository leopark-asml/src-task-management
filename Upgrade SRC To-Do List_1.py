...
def connect_to_database():
    try:
        conn_str = (
            ...
            r"SERVER=10.87.247.70\\SQLEXPRESS;"
            r"DATABASE=MyFirstDB;"
            r"UID=Service\MSSQL$SQLEXPRESS;"  # SQL 로그인 계정
            r"PWD=Service\MSSQL$SQLEXPRESS;"
        )
        conn = pyodbc.connect(conn_str)
        print("✅ 로컬 SQL Server 연결 성공!")
        return conn
...
def insert_default_user():
    ...
    # 사용자 존재 여부 확인
    cursor.execute('''
    SELECT COUNT(*) FROM Users WHERE LOWER(email) = LOWER(?) OR LOWER(four_letter) = LOWER(?)
    ''', ('leo.park@example.com', 'JXPX'))
    count = cursor.fetchone()[0]

    if count == 0:
        # 기본 사용자 추가
        cursor.execute('''
        INSERT INTO Users (email, password, approved, is_admin, four_letter)
        VALUES (?, ?, ?, ?, ?)
        ''', ('leo.park@asml.com', '990506qw!?', 1, 1, 'JXPX'))
        conn.commit()
        print("✅ 기본 사용자 정보가 추가되었습니다.")
    else:
        print("ℹ️ 기본 사용자 정보가 이미 존재합니다. 추가하지 않습니다.")
